...
void efp_to_efp6(efp6_t *ANS,efp_t *A){
  ANS->infinity=A->infinity;
  fp6_set_ui_ui(&ANS->x,0);
  fp6_set_ui_ui(&ANS->y,0);
  fp_set(&ANS->x.x0.x0,&A->x);
  fp_set(&ANS->y.x0.x0,&A->y);
}

void efp6_to_efp3(efp3_t *ANS,efp6_t *A){ //with twist
  ANS->infinity=A->infinity;
...
void efp3_to_efp6(efp6_t *ANS,efp3_t *A){
  ANS->infinity=A->infinity;
  fp_mul_base(&ANS->x.x0.x0,&A->x.x6); //with invert twist
  ...
  fp3_set_ui_ui(&ANS->x.x1,0);
  fp3_set_ui_ui(&ANS->y.x0,0);
...
void miller_ate_6sparse(fp6_t *f,efp6_t *P,efp6_t *Q){
  ...
  // efp3_to_efp6(&T14,&T);
  // efp6_println("T :",&T14);
  // efp6_scm(&tmp,Q,miller_loop_s);
  // efp6_println("Tmp :",&tmp);
  // getchar();
}
