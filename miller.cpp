...
void ff_lttp_0(fp6_t *f, efp_jacobian_t *S, efp_t *P){
  ...
  fp_mul(&f->x0.x1,&f->x0.x1,&P->y);     // = nextZ*Z^2Py
  fp_l1shift(&f->x0.x1,&f->x0.x1);      // = 2*nextZ*Z^2Py

  fp_mul(&tmp3_fp,&t3,&S->x);                        //tmp3 = t3*X
  fp_sub(&f->x0.x0,&tmp3_fp,&tmp2_fp);         // = t3*X - 2*t1

  fp_mul(&f->x2.x0,&t3,&P->x);                 // = t3*Px
  fp_mul(&f->x2.x0,&f->x2.x0,&tmp1_fp);  // = t3*Px*Z^2
  fp_set_neg(&f->x2.x0,&f->x2.x0);
...
//-----------------------------------------------
  ...
  fp_l1shift(&f->x0.x1,&f->x0.x1);
//-----------------------------------------------
  fp_l1shift(&f->x2.x0, &tmpA_fp);
  fp_add(&f->x2.x0,&f->x2.x0, &tmpA_fp); //3A
  fp_mul(&f->x2.x0,&f->x2.x0,&S->x);     //3ASx
//-----------------------------------------------
  // fp6_mul_sparse_dbl(f,&tmp1_fp6,f);        //Capable for further Karatsuba //update
  // fp6_mul(f,&tmp1_fp6,f);
...
//-----------------------------------------------
  fp_l1shift(&tmp1_fp6.x2.x0, &tmpA_fp);
  fp_add(&tmp1_fp6.x2.x0,&tmp1_fp6.x2.x0, &tmpA_fp); //3A
  fp_mul(&tmp1_fp6.x2.x0,&tmp1_fp6.x2.x0,&S->x);     //3ASx
//-----------------------------------------------
  fp6_mul_sparse_dbl(f,&tmp1_fp6,f);        //Capable for further Karatsuba //update
  // fp6_mul(f,&tmp1_fp6,f);
...
//-----------------------------------------------
  ...
  fp_add(&tmp1_fp6.x1.x0,&tmp1_fp,&tmp2_fp);  //Y2(X1-X2Z1) - X2(Y1-Y2Z1)
//-----------------------------------------------
  fp_mul(&tmp1_fp6.x1.x1,&tmpD_fp,&S->y);       //(X1-X2Z1)yS
  fp_set_neg(&tmp1_fp6.x1.x1,&tmp1_fp6.x1.x1);  //(X1-X2Z1)
...
void ff_lttp_lazy_montgomery_0(fp6_t *f, efp_jacobian_t *S, efp_t *P){
  ...
  fp_set_neg_montgomery(&f->x2.x0,&f->x2.x0);
...
void ff_lttp_costello_lazy_montgomery_0(fp6_t *f, efp_jacobian_t *U, efp_t *S){//U = S, S = P
  ...
  fp_l1shift(&tmpD_fp,&tmpD_fp);
  fp_l1shift(&tmpD_fp,&tmpD_fp);
  fp_set_neg_montgomery(&tmpD_fp,&tmpD_fp);
...
//-----------------------------------------------
  fp_l1shift(&f->x2.x0, &tmpA_fp);
  fp_add(&f->x2.x0,&f->x2.x0, &tmpA_fp); //3A
  fp_mulmod_montgomery(&f->x2.x0,&f->x2.x0,&S->x);     //3ASx
//-----------------------------------------------
  // fp6_mul_sparse_dbl_lazy_montgomery(f,&tmp1_fp6,f);        //Capable for further Karatsuba //update
  // fp6_mul(f,&tmp1_fp6,f);
...
//-----------------------------------------------
  fp_l1shift(&tmp1_fp6.x2.x0, &tmpA_fp);
  fp_add(&tmp1_fp6.x2.x0,&tmp1_fp6.x2.x0, &tmpA_fp); //3A
  fp_mulmod_montgomery(&tmp1_fp6.x2.x0,&tmp1_fp6.x2.x0,&S->x);     //3ASx
//-----------------------------------------------
  fp6_mul_sparse_dbl_lazy_montgomery(f,&tmp1_fp6,f);        //Capable for further Karatsuba //update
  // fp6_mul(f,&tmp1_fp6,f);
...
//-----------------------------------------------
  ...
  fp_add(&tmp1_fp6.x1.x0,&tmp1_fp,&tmp2_fp);  //Y2(X1-X2Z1) - X2(Y1-Y2Z1)
//-----------------------------------------------
  fp_mulmod_montgomery(&tmp1_fp6.x1.x1,&tmpD_fp,&S->y);       //(X1-X2Z1)yS
  fp_set_neg_montgomery(&tmp1_fp6.x1.x1,&tmp1_fp6.x1.x1);  //(X1-X2Z1)
  fp_l1shift(&tmp1_fp6.x1.x1,&tmp1_fp6.x1.x1);
//-----------------------------------------------
  fp6_mul_sparse_add_costello_montgomery(f,&tmp1_fp6,f); //update
  // fp6_mul(f,&tmp1_fp6,f);
