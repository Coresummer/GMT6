...
void final_exp_lazy_montgomery3(fp6_t *ANS,fp6_t *A){
  static fp6_t tmp1_fp6, tmp2_fp6,tmp3_fp6,tmp4_fp6,tmp5_fp6,tmp6_fp6,tmp7_fp6,tmp8_fp6,tmp9_fp6,tmp10_fp6,tmp11_fp6,tmp12_fp6;

  fp6_inv_lazy_montgomery(&tmp1_fp6,A);          //-1
  fp6_frobenius_map_p3_montgomery(ANS,A);        //(p^3)
  fp6_mul_lazy_montgomery(ANS,ANS,&tmp1_fp6);    //(p^3-1)
//If needed Use FP6_GS_SQR from here
  fp6_frobenius_map_p1_montgomery(&tmp1_fp6,ANS);//(p^3-1)(p)
  fp6_mul_lazy_montgomery(ANS,ANS,&tmp1_fp6);    //(p^3-1)(p+1) = M
  fp6_sqr_GS_lazy_montgomery(&tmp12_fp6, ANS);      //M^2   
  //Hard part
//L1
  fp6_frobenius_map_p3_montgomery(&tmp2_fp6,ANS);           //M^-1                                       //I
...
//L2
  //M'^3w //80
  fp6_finalexpow_3w_2NAF_lazy_montgomery(&tmp5_fp6, &tmp9_fp6);   //M'(3w)
  fp6_mul_lazy_montgomery(&tmp6_fp6,&tmp5_fp6,&tmp9_fp6);         //M'(3w+1)                                    //M
  
  //80
  fp6_finalexpow_3w_2NAF_lazy_montgomery(&tmp7_fp6, &tmp6_fp6);   //M'^9w^2 +3w
  fp6_mul_lazy_montgomery(&tmp11_fp6,&tmp7_fp6,&tmp9_fp6);        //M'^(9w^2+3w+1)                                //M
  //128
  fp6_finalexpow_x_1_2NAF_lazy_montgomery(&tmp11_fp6, &tmp11_fp6);//M'^(9w^2+3w+1)(x-1)

  fp6_mul_lazy_montgomery(&tmp6_fp6, &tmp7_fp6, &tmp5_fp6);       //(9w^2+6w)  
  fp6_mul_lazy_montgomery(&tmp11_fp6,&tmp11_fp6,&tmp6_fp6);       //M'^(9w^2+3w+1)(x-1)+(9w^2+6w)                 //M

  //128
  fp6_finalexpow_x_1_2NAF_lazy_montgomery(&tmp11_fp6, &tmp11_fp6);//M'^((9w^2+3w+1)(x-1)+9w^2+6w)(x-1)

  fp6_mul_lazy_montgomery(&tmp11_fp6,&tmp11_fp6,&tmp6_fp6);       //M'^((9w^2+3w+1)(x-1)+9w^2+6w)(x-1) +(9w^2+6w) //M
  fp6_mul_lazy_montgomery(&tmp11_fp6,&tmp11_fp6,&tmp5_fp6);       //M'^((9w^2+3w+1)(x-1)+9w^2+6w)(x-1) +(9w^2+9w) //M

  fp6_mul_lazy_montgomery(ANS,ANS,&tmp11_fp6);                    //M^(1 + (p-1+x)((9w^2+3w+1)(x-1)+9w^2+6w)(x-1)+9w^2+9w ) //M
  fp6_mul_lazy_montgomery(ANS,ANS,&tmp12_fp6);                    //M^(3 + (p-1+x)((9w^2+3w+1)(x-1)+9w^2+6w)(x-1)+9w^2+9w ) //M
}



void final_exp_lazy_montgomery(fp6_t *ANS,fp6_t *A){
  static fp6_t tmp1_fp6,tmp2_fp6,tmp3_fp6,tmp4_fp6,tmp5_fp6,tmp6_fp6;
