...
void fp6_mul_sparse_dbl_lazy_montgomery(fp6_t *ANS,fp6_t *A,fp6_t *B){  //??0?00 * ??????
  ...
  fp2_mul_nonmod_montgomery(&tmp2_fpd2,&tmp_A.x0,&tmp1_fp2); //a(d+e)

  //tmp1 = (a+c)(d+e+f)
  fp2_add_nonmod_single(&tmp3_fp2,&tmp_A.x0,&tmp_A.x2); //a+c
  fp2_add_nonmod_single(&tmp1_fp2,&tmp1_fp2,&tmp_B.x2); //d+e+f
  fp2_mul_nonmod_montgomery(&tmp1_fpd2,&tmp1_fp2,&tmp3_fp2); //(a+c)(d+e+f)

  //tmp3 = ad
  fp2_mul_nonmod_montgomery(&tmp3_fpd2,&tmp_A.x0,&tmp_B.x0);

  //tmp4 = ce
  fp_mul_nonmod(&tmp4_fpd2.x0,&tmp_B.x1.x0,&tmp_A.x2.x0);
  fp_mul_nonmod(&tmp4_fpd2.x1,&tmp_B.x1.x1,&tmp_A.x2.x0);

  //tmp5 = cf
  fp_mul_nonmod(&tmp5_fpd2.x0,&tmp_B.x2.x0,&tmp_A.x2.x0);
  fp_mul_nonmod(&tmp5_fpd2.x1,&tmp_B.x2.x1,&tmp_A.x2.x0);

  //x0 = 3+4beta^3
  fp2_mul_base_nonmod_double(&tmp1,&tmp4_fpd2);
  fp2_add_nonmod_double(&tmp1,&tmp1,&tmp3_fpd2);
  fp2_mod_montgomery_double(&ANS->x0, &tmp1);

  //x1 = 2-3 + 5beta^3
  fp2_mul_base_nonmod_double(&tmp2,&tmp5_fpd2);
  fp2_add_nonmod_double(&tmp2,&tmp2,&tmp2_fpd2);
  fp2_sub_nonmod_double(&tmp2,&tmp2,&tmp3_fpd2);
  fp2_mod_montgomery_double(&ANS->x1, &tmp2);

  //x2 = 1-2-5-4
  fp2_sub_nonmod_double(&tmp3,&tmp1_fpd2,&tmp2_fpd2);
  fp2_sub_nonmod_double(&tmp3,&tmp3,&tmp4_fpd2);
